...
function processCommand(command) {
    ...
    switch (command[0]) {
        case 'show':            
            for (const todo of makeTODOTable(todos))
                console.log(todo);
            break;
        case 'important':
            todos.forEach(element => {
                if (element.com.includes("!"))
                    result.push(element);
            });
            for (const element of makeTODOTable(result))
                console.log(element);
            break;
        case 'user':
            ...
            for (const todo of makeTODOTable(result))
                console.log(todo);
            break;
        case 'sort':
...
function makeTODOTable(todos){
    ...
    let result = todos
    .map(value=> value = `  ${value.com.includes("!")? "!": " "}  |`+
    `  ${value.hasOwnProperty('name') ? value.name.length <= lengths.name ? value.name.padEnd(lengths.name): value.name.substring(0,lengths.name - 3).padEnd(lengths.name, ".") : "".padEnd(lengths.name)}  |`+
    `  ${value.hasOwnProperty('date') ? value.date.length <= lengths.date ? value.date.padEnd(lengths.date): value.date.substring(0,lengths.date - 3).padEnd(lengths.date, ".") : "".padEnd(lengths.date)}  |`+
    `  ${value.com.length <= lengths.com ? value.com.padEnd(lengths.com) : value.com.substring(0, lengths.com - 3).padEnd(lengths.com, ".")}`);
    let lending = `  !  |  ${"user".padEnd(lengths.name)}  |  ${"date".padEnd(lengths.date)}  |  ${"comment".padEnd(lengths.com)}  `;
...
function sortTodos(sortItem, todos){
    switch (sortItem){
        case 'importance':
            return todos.sort((a,b) => (b.com.match(/!/g) || []).length - (a.com.match(/!/g) || []).length);
        case 'user':
