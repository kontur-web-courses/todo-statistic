...
function processCommand(command) {
    switch (command) {
        ...
        case command.match(/^((date [0-9]{4}-[0-9]{2}-[0-9]{2}|date [0-9]{4}-[0-9]{2})|date [0-9]{4})$/g) ? command : null:
            let date = new Date(command.match(/(([0-9]{4}-[0-9]{2}-[0-9]{2}|[0-9]{4}-[0-9]{2})|[0-9]{4})$/g)[0]);
            console.log(
                allTodos
...
function findTodos(files) {
    ...
    todos = todos.map(todo => {
        let arr = todo.comment.replace(/\/\/\s?todo(\s|:)/gi, '')
        .split(';')
        ...
        if (arr.length === 1) {
            newTodo.message = arr[0].trim();
        } else if (arr.length === 3) {
            let user = arr[0].trim();
            newTodo.user = user && user.length > 0 ? user : null;
            let dateArray = arr[1].trim().split('-');
            let date = new Date(arr[1].trim());
            newTodo.date = {
                date: date,
